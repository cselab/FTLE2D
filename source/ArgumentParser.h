...
public:
	...
	ArgumentParser(const int argc, const char ** argv) : mapArguments(), iArgC(argc), vArgV(argv), bStrictMode(false), bVerbose(false)
	{
		...
		if (bVerbose) printf("found %ld arguments of %d\n",mapArguments.size(),argc);
	}
